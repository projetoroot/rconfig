...
detect_os() {
    ...
    if [[ "${ID}" != "debian" ]]; then
        warn "Este instalador foi desenvolvido para Debian 13."
        read -r -p "Deseja continuar mesmo assim? [s/N]: " ANSWER
        [[ "${ANSWER,,}" == "s" ]] || exit 0
    fi
...
install_gems() {
    ...
    # -------------------------------------------------------------------------
    # Diretório de executáveis RubyGems
    # -------------------------------------------------------------------------
    ...
    if [[ -z "${gem_bin_dir}" || ! -d "${gem_bin_dir}" ]]; then
        die "Não foi possível determinar o diretório de executáveis do RubyGems."
    fi

    export PATH="${gem_bin_dir}:${PATH}"

    log "Diretório de executáveis RubyGems: ${gem_bin_dir}"

    # -------------------------------------------------------------------------
    # Verifica o executável principal
    # -------------------------------------------------------------------------
    ...
    log "Oxidized: ${gem_bin_dir}/oxidized"

    # -------------------------------------------------------------------------
    # oxidized-web
    #
    # Nem toda instalação da gem cria um executável chamado oxidized-web.
    # O Web pode ser iniciado pelo próprio comando oxidized com a configuração
    # correspondente.
    # -------------------------------------------------------------------------

    if [[ -x "${gem_bin_dir}/oxidized-web" ]]; then
        log "Oxidized Web: ${gem_bin_dir}/oxidized-web"
    else
        warn "A gem oxidized-web não fornece um executável oxidized-web em ${gem_bin_dir}."
        log "Gem oxidized-web instalada e disponível para o Oxidized."
    fi

    # -------------------------------------------------------------------------
    # Verificação das gems instaladas
    # -------------------------------------------------------------------------

    if ! gem list '^oxidized$' -i -v "${OXIDIZED_VERSION}" >/dev/null 2>&1; then
        die "A gem oxidized ${OXIDIZED_VERSION} não foi encontrada."
    fi

    if ! gem list '^oxidized-web$' -i -v "${OXIDIZED_WEB_VERSION}" >/dev/null 2>&1; then
        die "A gem oxidized-web ${OXIDIZED_WEB_VERSION} não foi encontrada."
    fi
...
backup_file() {
    ...
    chown "${OX_USER}:${OX_GROUP}" "${OX_BACKUP_DIR}"/*
    chmod 600 "${OX_BACKUP_DIR}"/*
}
...
configure_web() {
    ...
    echo "  ${HOST_IP}"
    echo
    echo "Também é possível usar 0.0.0.0, mas isso expõe o Web em todas"
    echo "as interfaces e não é recomendado sem firewall/reverse proxy."
    echo
...
# Remove segredos suportados pelos modelos.
vars:
  remove_secret: true
...
create_temp_node() {
    ...
    if grep -qE "^${TEMP_NAME}:" "${OX_ROUTER_DB}" 2>/dev/null; then
        return 0
    fi
...
if ! awk -F: -v name="${NAME}" '$1 == name { found=1 } END { exit !found }' "${DB}"; then
...
awk -F: -v name="${NAME}" '$1 != name { print }' "${DB}" > "${TMP}"
...
LINE="$(
    awk -F: -v name="${NAME}" '$1 == name { print; exit }' "${DB}"
)"
...
if ! model_exists "${MODEL}"; then
    ...
    get_model_dir | xargs -I{} find {} -maxdepth 1 -type f -name '*.rb' \
        -printf '%f\n' | sed 's/\.rb$//' | sort
    exit 1
fi
...
if KEYSCAN_OUTPUT="$(
    ...
    if [[ -n "${KEYSCAN_OUTPUT}" ]]; then
        ...
        cat /var/lib/oxidized/.ssh/known_hosts 2>/dev/null > "${TMP_KNOWN}" || true
        printf '%s\n' "${KEYSCAN_OUTPUT}" >> "${TMP_KNOWN}"

        sort -u "${TMP_KNOWN}" > /var/lib/oxidized/.ssh/known_hosts

        rm -f "${TMP_KNOWN}"

        chown oxidized:oxidized /var/lib/oxidized/.ssh/known_hosts
        chmod 600 /var/lib/oxidized/.ssh/known_hosts
...
###############################################################################
# REMOVE TEMPORARIO APOS PRIMEIRO EQUIPAMENTO REAL
###############################################################################
...
if (( REAL_COUNT > 0 )); then
    ...
    awk -F: -v name="${TEMP_NAME}" '$1 != name { print }' "${DB}" > "${TMP2}"
...
if ! systemctl is-active --quiet oxidized; then
    ...
    echo
    systemctl status oxidized --no-pager -l || true
    exit 1
fi
...
set_permissions() {
    ...
    if [[ -d "${OX_SSH_DIR}" ]]; then
        chown -R "${OX_USER}:${OX_GROUP}" "${OX_SSH_DIR}"
        chmod 700 "${OX_SSH_DIR}"

        if [[ -f "${OX_KNOWN_HOSTS}" ]]; then
            chmod 600 "${OX_KNOWN_HOSTS}"
            chown "${OX_USER}:${OX_GROUP}" "${OX_KNOWN_HOSTS}"
        fi
    fi
    ...
    if [[ -d "${OX_GIT_REPO}" ]]; then
        chown -R "${OX_USER}:${OX_GROUP}" "${OX_GIT_REPO}"
    ...
    # =========================================================================
    # Não devem pertencer ao usuário oxidized.
    # São executáveis administrativos do sistema.
...
show_status() {
    ...
    echo
    systemctl --no-pager --full status oxidized || true
...
main() {
    ...
    # SCRIPTS AUXILIARES
    # =========================================================================
    # Os helpers precisam existir antes de qualquer tentativa de alterar
    # suas permissões/ownership.
